...
RSpec.describe Publication, type: :request do
  # Se crea una instancia de User para mockear las relaciones
  ...
  describe 'update' do
    it 'should change a Publication' do
      expect do
        login_as user
        patch "/publications/#{publication.id}", params: { publication: { title: 'Ejemplo para cambiar' } }
  ...
  describe 'update' do
    it 'should not change a Publication' do
      expect do
        patch "/publications/#{publication.id}", params: { publication: { content: 'hola', title: 'example' } }
        # Se recarga la instancia de publication nuevamente con los posibles nuevos atributos
